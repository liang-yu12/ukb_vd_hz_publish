...
# Crude model -----
hz_vdw_crude <- survreg( Surv(fu_yr, hz_se) ~ vitd_s, 
...
# fixed +/-
hz_vdw_crude_tidy[,-1] <- hz_vdw_crude_tidy[,-1]*-1
...
# Model
hz_vdw_partial <-  survreg( Surv(fu_yr, hz_se) ~ vitd_s + sex + age_c, 
                            data = bd_i, dist = "weibull")
...
# model
hz_vd.weibull <- survreg( Surv(fu_yr, hz_se) ~ vitd_s + sex + age_c + ethnic + 
...
# combine ----
fig2_data_se <- bind_rows(crude_w, partial_w, full_w)
fig2_data_se %>% write_csv("dataset_wd/fig2_data_se_weibull.csv")
# fig2_data <- read_csv("dataset_wd/fig2_data_weibull.csv")

# Further data management for forestplot ----

# a: vd status:
fig2_data_se_w <- fig2_data_se  %>% 
...
fig2_data_se_w$order <- 9:1

# Plotting: -----
# central figure 
fig2_plot_w <- fig2_data_se_w %>% 
