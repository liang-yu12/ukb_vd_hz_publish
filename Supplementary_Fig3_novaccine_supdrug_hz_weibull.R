...
# a. all_supp ----
# crude model -----
hz_supp_crude_w <- survreg(Surv(fu_yr, hz_se) ~all_supp, data = bd_i, dist = "weibull")
...
hz_vdsupp.partial <- survreg(Surv(fu_yr, hz_se) ~all_supp + sex + age_c, 
...
hz_vdsupp_full.w <- survreg(Surv(fu_yr, hz_se) ~all_supp + sex + age_c + ethnic + 
...
fig3a_data_se <- bind_rows(supp_crude, supp_partial, supp_full)

# save file
fig3a_data_se %>% write_csv("dataset_wd/supp_fig2a_data_weibull.csv")
# fig3a_data <- read_csv("dataset_wd/fig3a_data_weibull.csv")

# data manipulation for figure ------
fig3a_data_se_w <- fig3a_data_se %>% 
...
# crude ----
vd_drugcrude.w <- survreg(Surv(fu_yr, hz_se) ~vd_prescription, 
                          data = bd_i, dist = "weibull")
...
# model:
vd_drug_partial.w <- survreg(Surv(fu_yr, hz_se) ~vd_prescription + sex + age_c, 
                             data = bd_i, dist = "weibull")
# tidy 
...
# full ----
# model
vd_drug_full.w <- survreg(Surv(fu_yr, hz_se) ~vd_prescription + sex + age_c + ethnic + 
                                bmi_group + drink_freq_c + smoke_stat + imd_bd_q + regions +
                                season_c + asthma + ckd+ copd + depress + dm + ibd + ra + sle + immunosuppression, 
                          data = bd_i, dist = "weibull")
# tidy 
...
# combine data -----
fig3b_data_se <- bind_rows(drug_crude, drug_partial, drug_full)
# save files
fig3b_data_se %>% write_csv("dataset_wd/fig3b_data_weibull.csv")
# fig3b_data <- read_csv("dataset_wd/fig3b_data_weibull.csv")

# manipulation for plotting -----
fig3b_data_se_w <- fig3b_data_se %>% 
